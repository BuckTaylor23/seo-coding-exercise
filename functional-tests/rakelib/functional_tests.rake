...
namespace :functional_tests do
  ...
  task :run, %i[processes feature_path tag] do |_t, args|
    ...
    cmd = %W[ -n #{args.processes} --type cucumber --group-by scenarios --serialize-stdout
              -- -f #{FORMATTER} --out /dev/null -f pretty -t #{tags}
              -- #{args.feature_path} ]
  ...
  task :blitz, %i[repetitions tags] do |_t, args|
    args.with_defaults(repetitions: 1, tags: nil)

    tags = args.tags.nil? ? 'not @exclude' : "#{args.tags} and not @exclude"

    LOG.info { "Running blitz: #{args[:repetitions]} times using tags: #{tags}" }

    args[:repetitions].to_i.times { sh "bundle exec cucumber -t '#{tags}'" }
  end
end
